...
3. **Automated Browser Fallback (MCP only):** If the HTTP path returns `SERVICE_DISABLED` for `serviceusage.googleapis.com` or `apikeys.googleapis.com`, the agent must use `skylight-cli` MCP browser automation, not ask the end user to click anything. The production fallback order is:
...
10. For complex edits/generations, inspect outputs (open/view images) and validate: subject, style, composition, text accuracy, and invariants/avoid items.
11. Iterate: make a single targeted change (prompt or mask), re-run, re-check.
12. **Apply Watermark:** Run `python ~/.config/opencode/skills/imagegen/scripts/add_watermark.py <input> <output> ~/.config/opencode/skills/imagegen/aiometrics_logo.png` to apply the transparent AIometrics logo.
13. Save/return final outputs and note the final prompt + flags used.
...
- **`references/cli.md`**: how to _run_ image generation/edits/batches via `scripts/gemini_image_router.py` (commands, flags, env, recipes).
