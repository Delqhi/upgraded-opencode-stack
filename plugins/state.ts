...
export function pathsOverlap(path1: string, path2: string): boolean {
  const n1 = path1.replace(/\\/g, '/').replace(/\/+$/, '');
  const n2 = path2.replace(/\\/g, '/').replace(/\/+$/, '');
...
function findPathConflict(
  ...
  for (const entry of current) {
    if (entry.swarmId === reservation.swarmId) continue;
    if (entry.mode === 'exclusive') {
      for (const newPath of normalizedPaths) {
        for (const existingPath of entry.paths) {
          if (pathsOverlap(newPath, existingPath)) return entry;
        }
      }
    }
    if (entry.mode === 'shared' && reservation.mode === 'exclusive') {
      for (const newPath of normalizedPaths) {
        if (entry.paths.some((existingPath) => existingPath === newPath)) return entry;
      }
    }
  }
  return undefined;
...
export async function reserveSwarmPaths(
  ...
  if (conflict) return { ok: false, conflict, reservations: current };
  const now = Date.now();
